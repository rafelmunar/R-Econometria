...
#**Apartado a**: Haz una regresión lineal múltiple del peso de los bebés recién nacidos (bwghti) frente al número de cigarrillos fumados por día por la madre durante el embarazo (cigsi) y a los ingresos familiares anuales (faminci):

reg11 <- lm(bwght ~ cigs + faminc, data = bwght)

summary(reg11)

sum.reg11 <- summary(reg11)

#**Apartado b**: Considera ahora como variable explicativa los paquetes de cigarrillos fumados por día por la madre durante el embarazo (packsi) y. Estima por Mínimos Cuadrados Ordinarios el modelo (2) abajo. Se dice que hay multicolinealidad perfecta cuando el valor absoluto de la correlación entre dos regresores es mayor que 0,7. ¿Hay Multicolinealidad Perfecta para algún par de regresores del modelo (2)? ¿Qué harías para corregir este problema?

reg12 <- lm(bwght ~ cigs + faminc + packs, data = bwght)

summary(reg12)

sum.reg12 <- summary(reg12)

cor(bwght$cigs, bwght$packs)

#**Apartado c**: Considera ahora como variable explicativas los años de estudio de la madre (motheduci) y el orden de nacimiento del bebé (parityi). Estima por Mínimos Cuadrados Ordinarios los modelos (3) y (4) abajo. Según los modelos (3) y (4), al 5 % de significación, ¿los años de estudio de la madre y el orden de nacimiento del bebé afectan el peso de un bebé recién nacido?

reg13 <- lm(bwght ~ cigs + faminc + motheduc, data = bwght)

summary(reg13)

sum.reg13 <- summary(reg13)

reg14 <- lm(bwght ~ cigs + faminc + motheduc + parity, data = bwght)

summary(reg14)

sum.reg14 <- summary(reg14)

#**Apartado d**: Considera ahora como variable explicativa una variable binaria que toma 1 si el bebé es varón y 0 en caso contrario (malei). Estima por Mínimos Cuadrados Ordinarios el modelo (5) abajo. ¿Es significativo el coeficiente de la variable explicativa malei al 5 % de significación? Haz un intervalo de confianza del 99 % para los coeficientes estimados.

reg15 <- lm(bwght ~ cigs + faminc + motheduc + parity + male, data = bwght)

summary(reg15)

sum.reg15 <- summary(reg15)

confint(reg15, level = 0.99)

anova(reg11, reg13)

#**Apartado e**: Compara los modelos (1), (3), (4) y (5). Selecciona el mejor modelo

res.ftest <- var.test(sum.reg11$r.squared, sum.reg13$r.squared)
